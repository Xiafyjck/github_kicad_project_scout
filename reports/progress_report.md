...
改动的最小单位定为事件：一次改动 × 一个工程目录。PR 事件只在 PR 改了至少一个 KiCad 文件时产生，按每个 KiCad 文件所在的工程目录分条，一个 PR 改了 3 个目录的 KiCad 文件就是 3 条；commit 事件来自"按工程目录查询提交"接口，该接口返回目录下任意文件有改动的提交，所以 README、gerber 的改动也会产生 commit 事件，有 142900 条事件没碰任何 KiCad 文件，这一点在底线"改了 .kicad_pcb"那一步统一处理。每个事件要定两个版本号：改动前版本用来 checkout 出题目输入的工程，改动后版本用来 checkout 出参考答案。PR 事件的改动前版本是 PR 所基于的主干提交（base），改动后版本是合并后的提交（GitHub 列表接口常不返回，这时用 PR 分支上最后一次提交代替）；commit 事件的改动前版本是该提交的父提交，改动后版本是该提交本身。每条事件记录改动的 KiCad 文件清单及其状态（新增 / 修改 / 改名 / 删除）、增删行数、按 pcb / sch / pro 的计数，以及说明文字里 `#N` 引用并确认存在的 issue。
...
| 改进事件 | 一次改动 × 一个工程目录。每个事件有"改动前版本"和"改动后版本"两个提交号，checkout 前者得到题目输入的工程，checkout 后者得到参考答案。PR 事件：PR 改了至少一个 KiCad 文件才记，按 KiCad 文件所在的工程目录分条（改 3 个目录记 3 条）。commit 事件：按工程目录查询到的每个提交都记，目录下任意文件改动都算，是否改了 KiCad 文件到底线再判。每个事件带改动前后版本、落在该目录的 KiCad 文件清单与状态、按 pcb / sch / pro 的文件数、逐行差异、引用的 issue | 523908 个事件（PR 事件 33844，commit 事件 490064） |
...
flowchart TB
    ...
    C[候选仓库 69181]:::keep --> A[分析仓库 39902]:::keep --> Q[完整工程仓库 32029<br/>工程目录 66793]:::keep --> E[改进事件 523908]:::keep
    C -.-> CX([未分析 29279]):::exit
    A -.-> AX([删除 4 / 截断 21 / 不完整 7848]):::exit
    E --> P[PR 事件 33844]:::keep
    E --> M[commit 事件 490064]:::keep

    P --> P3[底线 12152]:::keep --> P4[文字 3534]:::keep --> P5[规模 1066]:::keep --> P6[全为修改 855]:::keep --> P7[有 patch 511]:::keep --> P8[真实改动 328]:::keep --> P9[限额 283]:::keep
    ...
    M --> M3[底线 275885]:::keep --> M4[文字 23086]:::keep --> M5[规模 10456]:::keep --> M6[全为修改 8444]:::keep --> M7[有 patch 4398]:::keep --> M8[真实改动 3957]:::keep --> M9[限额 3416]:::keep
    M -.-> MX0([无 pcb 改动 214179]):::exit
    M3 -.-> MX4([252799]):::exit
    M4 -.-> MX5([12630]):::exit
    M5 -.-> MX6([2012]):::exit
    M6 -.-> MX7([4046 C 级]):::exit
    M7 -.-> MX8([441]):::exit
    M8 -.-> MX9([541]):::exit

    P9 --> F[最终池 3699<br/>1441 仓库]:::final
    M9 --> F
    F --> G[A 级 200]:::final
    F -.-> FB([B 级 3499]):::exit
```

讲解：从左上到右下是一条漏斗。仓库层三步：四种后缀搜索并集得到候选仓库 69181；只对第一版 `.kicad_pro` 命中的 39902 个做了深度分析，其余 29279 个等 Release C；拉完整文件树后 32029 个仓库至少有一个完整工程目录。事件层从这 32029 个仓库的历史里得到 523908 个改进事件，分 PR 与 commit 两支。每支先过底线（工程目录内、PR 已合并、改了 .kicad_pcb），再过五道筛：文字、规模、全为修改、有逐行差异、真实改动，最后按仓库限额。PR 支从 33844 收到 283，commit 支从 490064 收到 3416，合并成最终池 3699 个事件、1441 个仓库，其中引用了 issue 的 200 个为 A 级。每一层旁边的灰框是被筛掉的数量，同一层相邻两个数字相减就是出口。
...
flowchart TB
    ...
    Q --> H
    H --> E[改进事件 523908<br/>一次改动 × 一个工程目录]:::keep
    E --> D[记：KiCad 文件与状态、增删行、引用的 issue]:::rule
    D --> PR[PR 事件：改了 KiCad 文件才记<br/>前 base，后合并提交]:::rule
    D --> CM[commit 事件：目录下任意改动都记<br/>前父提交，后自身]:::rule
    PR --> P[PR 事件 33844]:::keep
    CM --> M[commit 事件 490064]:::keep
...
flowchart TB
    ...
    Rb -.-> TX
    Rc -.-> PX5([过大 2468]):::exit
    Rd -.-> PX6([新增 / 改名 / 删除 211]):::exit
    Re -.-> PX7([无 patch 344, C]):::exit
    P7 -.-> PX8([只存盘 183]):::exit
    Rf -.-> PX9([超限额 45]):::exit
```

讲解：从 12152 个底线 PR 事件开始，五道筛各解决一个问题。文字筛管有没有题面：标题加正文去掉 checklist、HTML 注释、引用、签名后至少 100 字，筛掉 8618，子原因可重叠，太短 8016、PR 模板 585、纯 issue 编号或链接 149、TODO 为主 2，剩 3534。规模筛管是不是一次可理解的修改：KiCad 文件增删行合计不超过 8000、改动文件不超过 40，筛掉 2468，剩 1066。修改筛管前后状态能否对上：所有 KiCad 文件必须是 modified，含新增、改名、删除的筛掉 211，剩 855。存盘筛管是不是真改了电路：先要有逐行差异，GitHub 对过大文件不返回 patch，344 条记 C 级待定，剩 511；再去掉 uuid、tstamp、version、generator 这类存盘就会变的行，真实改动行至少 5 行，只有存盘 churn 的 183 条筛掉，剩 328。限额筛管少数仓库垄断：每仓库最多 20 条，优先有 issue 关联、说明长的，超限额 45 条，剩 283 进入最终池。
...
flowchart TB
    ...
    M7 -.-> MX7([无 patch 4046, C]):::exit
    M8 -.-> MX8([只存盘 441]):::exit
    M9 -.-> MX9([超限额 541]):::exit

    P9[PR 支路 283]:::keep
    M9 --> F[最终池 3699<br/>1441 仓库]:::final
    P9 --> F
    F --> G[A 级 200，引用 issue]:::final
    F -.-> FB([B 级 3499]):::exit
    MX7 -.-> FC([C 级 4390 待定]):::exit
```
