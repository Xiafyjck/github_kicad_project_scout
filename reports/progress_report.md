...
| 改进事件 | 一次改动 × 一个工程目录。PR 事件：PR 改了至少一个 KiCad 文件才记，按 KiCad 文件所在的工程目录分条（改 3 个目录记 3 条）。commit 事件：按工程目录查询到的每个提交都记，目录下任意文件改动都算，是否改了 KiCad 文件到底线再判。每个事件带改动前后版本、落在该目录的 KiCad 文件清单与状态、按 pcb / sch / pro 的文件数、逐行差异、引用的 issue | 523908 个事件（PR 事件 33844，commit 事件 490064） |
